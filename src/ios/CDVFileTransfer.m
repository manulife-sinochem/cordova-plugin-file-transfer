...
@property (nonatomic, assign) NSInteger currentBytes;
...
- (void)abort:(CDVInvokedUrlCommand*)command
{
    ...
    @synchronized (activeTransfers) {
        ...
        if (delegate != nil) {
            self.currentBytes = delegate.bytesTransfered;
            self.totalBytes = delegate.bytesExpected;
            self.isLoading = NO;
...
//下载完成后通知
- (void)downloadingComplete:(NSNotification *)noti{
...
- (void)download:(CDVInvokedUrlCommand*)command
{
    ...
    [center addObserver:self selector:@selector(downloadingComplete:) name:@"downloading" object:nil];
    ///Change
    ...
    //改文件名为同一个
//    NSArray *arr = [target componentsSeparatedByString:@"/"];
//    NSMutableArray *muArr = [arr mutableCopy];
//
//    [muArr removeObjectAtIndex:muArr.count-1];
//    [muArr addObject:@"adsadsadasdadasda.mp4"];
//
//    target = [muArr componentsJoinedByString:@"/"];
//
    ...
    //计算文件大小
    long size = [self fileSizeAtPath:[self targetFilePath:target]];
    
    // 设置请求头
    self.currentBytes = size;
    delegate.bytesTransfered = self.currentBytes;
    delegate.bytesExpected = self.totalBytes;
    NSString *range = [NSString stringWithFormat:@"bytes=%ld-", (long)self.currentBytes];
    [req setValue:range forHTTPHeaderField:@"Range"];
    
    //防止开始下载连续点击
    if ([source isEqualToString:self.sourceUrl]&&self.isLoading) {
...
- (NSString *)targetFilePath:(NSString *)target
{
    NSString *path = nil;
    CDVFilesystemURL *sourceURL = [CDVFilesystemURL fileSystemURLWithString:target];
    if (sourceURL && sourceURL.fileSystemName != nil) {
        // This requires talking to the current CDVFile plugin
        NSObject<CDVFileSystem> *fs = [[self.commandDelegate getCommandInstance:@"File"] filesystemForURL:sourceURL];
        path = [fs filesystemPathForURL:sourceURL];
    } else {
        // Extract the path part out of a file: URL.
        path = [target hasPrefix:@"/"] ? [target copy] : [(NSURL *)[NSURL URLWithString:target] path];
    }
    return path;
}
...
- (void)connectionDidFinishLoading:(NSURLConnection*)connection
{
    ...
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        ...
        NSNotification * notice = [NSNotification notificationWithName:@"downloading" object:nil userInfo:@{@"status":@"finish"}];
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse*)response
{
    ...
    ///Change
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {//断点任务还未下载完成
        self.bytesExpected = [response expectedContentLength] + self.bytesTransfered;
        if (self.bytesTransfered > 0){
...
- (void)connection:(NSURLConnection*)connection didFailWithError:(NSError*)error
{
    ...
    NSNotification * notice = [NSNotification notificationWithName:@"downloading" object:nil userInfo:@{@"status":@"fail"}];
    [[NSNotificationCenter defaultCenter]postNotification:notice];
